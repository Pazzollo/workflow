...
<div class="d-flex gap-3" id="flexbox">
    ...
    <a href="{{ route('transfer.display', ['year' => $year, 'company' => $company, 'transfer' => $_GET['transfer']]) }}" class="btn {{ $years[0] == $year ? 'btn-primary' : 'btn-secondary' }} mt-3"
        style="min-width: 100px">{{ $year }}</a>
    @endforeach
    <a href="{{ route('transfer.display', ['company' => $company, 'transfer' => $_GET['transfer'], 'year' => 'all']) }}" class="btn btn-secondary mt-3">Sve godine</a>
