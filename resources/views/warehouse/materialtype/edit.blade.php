...
@section('content')
    ...
    <main class="d-flex" id="main">
        ...
        <content id="content" class="mt-2 px-1">
            ...
            <form action="{{ route('material_type.update', $materialType) }}" method="post">
                @csrf
                @method('PUT')
                <div>
                    <div class="w-100 mb-3">
                        <label for="name" class="form-label mx-1">Naziv vrste materijala</label>
                        <input type="text" class="form-control" name="name" value="{{ $materialType->name }}">
                    </div>
                </div>

                <x-footer>
                    @if (auth()->user()->role_id == 1)
                        <button class="btn btn-sm w-100 btn-outline-dark border-1 mb-2" type="submit">
