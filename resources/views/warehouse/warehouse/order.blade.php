...
@php
    $title = 'Materijal za poručivanje';
@endphp
        ...
        <content id="content" class="mt-2 px-1">
            ...
            <div id="scroll">
                @foreach ($materials as $material)
                    @if ($quantities->where('material_id', $material->id)->sum('quantity') - $reservations->where('material_id', $material->id)->where('reserved', 1)->sum('quantity') <= 0)
                        {{-- Ako je stanje materijala sa rezervacijama negativno, ne prikazujemo ga --}}
